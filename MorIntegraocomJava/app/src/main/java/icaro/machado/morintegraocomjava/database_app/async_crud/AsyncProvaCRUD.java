package icaro.machado.morintegraocomjava.database_app.async_crud;
...
import icaro.machado.morintegraocomjava.database_app.dbcallbacks.IProvaDbCallback;
import icaro.machado.morintegraocomjava.database_app.tabelas.Prova;
import icaro.machado.morintegraocomjava.utils_app.UtilsApp;
    ...
    protected List<Prova> doInBackground(Prova... provas) {
        try{
            ...
            switch (dbOperations){
                ...
                case UPDATE:{
                    databaseApp.provasDAO().updateProvas(provas[0]);
                    break;
                }
                case DELETE:{
                    databaseApp.provasDAO().deleteProvas(provas[0]);
                    break;
                }
            }
        } catch (Exception e){
